...
import ra.rta.rfm.models.WANDType;
import ra.rta.rfm.models.WANDVehicle;
import ra.rta.utilities.DateUtility;
import ra.rta.classify.ClassificationService;
...
public class TransactionClassifier implements Classifier {
	...
	private static Logger LOG = LoggerFactory.getLogger(ra.rta.classify.TransactionClassifier.class);

	@Override
	public void classify(Classifiable classifiable, LoadingCache<String, LinkedHashSet<RFMKPI>> exactMatchTermcodeCache) throws Exception {
		if(classifiable instanceof TransactionEvent) {
			...
			try {
				LinkedHashSet<RFMKPI> cachedTermcodes = exactMatchTermcodeCache.get(eventIn.financialTransaction.payee);
				for (RFMKPI kpi : cachedTermcodes) {
					eventIn.financialTransaction.kpis.add((RFMKPI) kpi.clone());
				}
