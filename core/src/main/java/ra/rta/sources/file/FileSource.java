...
public class FileSource implements Runnable {
	...
	public void run() {
		LOG.info("{} starting...", FileSource.class.getSimpleName());
		try {
...
//			WatchKey key = sourceDir.register(watcher, StandardWatchEventKinds.ENTRY_CREATE);
            ...
            LOG.error("IOException in " + FileSource.class.getSimpleName() +": "+ ex);
            return;
        }
        LOG.info("{} normal shutdown completed.", FileSource.class.getSimpleName());
	}
