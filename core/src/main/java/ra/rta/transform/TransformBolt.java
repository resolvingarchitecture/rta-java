...
public class TransformBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		try {
			eventIn = MAPPER.readValue(bytes, Event.class);
			try {
				Transformer transformer = transformerFactory.build(eventIn);
				Event eventOut = transformer.transform(eventIn);
	...
	private void load() {
		try {
			...
			Map<String, Cluster> allActivePartnersMap = DataServiceManager.getGroupDataService()
