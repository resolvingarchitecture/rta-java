...
//        LOG.info("transform event: "+event);
		...
		raw = event.getEnvelope().getBody().getRecords().get(0).getRaw();
		select();
		clean();
        concatenate();
//		split();
//		format();
		substitute();
		map();
		event.getEnvelope().getBody().getRecords().get(0).setTransformed(true);
	}
...
//                LOG.info("attribute_name: "+attributeName+", value: "+fieldValue);
		...
		beanUtilsBean.populate(entity, fieldValuesMapped);
	}
