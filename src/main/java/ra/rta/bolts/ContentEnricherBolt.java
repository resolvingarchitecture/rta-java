...
//		maxRetries = (Long) map.get("topology.bolts.contentenricher.retries.max");
	...
	public void execute(Event event) throws Exception {
		if (event instanceof EnrichableEvent) {
			try {
				enricherService.enrich((EnrichableEvent) event);
			} catch (UCIDNotProvidedException e) {
				EventException error = new EventException(ContentEnricherBolt.class.getSimpleName(), 104, event.getId().toString(), event);
