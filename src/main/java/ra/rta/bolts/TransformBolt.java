...
public class TransformBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		try {
			envelope = MAPPER.readValue(new ByteArrayInputStream(bytes), Envelope.class);
			try {
				Event event = transformService.transform(envelope);
				String description = ((Transaction)event.getEntity()).getPayee();
	...
	private void load() {
		try {
			...
			Map<String, Partner> allActivePartnersMap = DataServiceManager.getPartnerDataService()
