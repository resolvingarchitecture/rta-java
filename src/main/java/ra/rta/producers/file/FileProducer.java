                ...
                for (Path path : oldFiles) {
                    ...
                    if (path.toString().contains(".mkr")) {
                        ...
                        if (dataFile == null) {
                            ...
                            FileSplitter fileSplitter = new FileSplitter(partnerNamespace, dataFile.getFileName(), markerFile.getFileName(), startingLine, sourceDir, archiveDir, messageManager);
...
//			WatchKey key = sourceDir.register(watcher, StandardWatchEventKinds.ENTRY_CREATE);
            ...
            for (; ; ) {
                ...
                for (WatchEvent<?> event : key.pollEvents()) {
                    ...
                    FileSplitter fileSplitter = new FileSplitter(partnerNamespace, dateFileName, markerFileName, 0, sourceDir, archiveDir, messageManager);
                    fileSplitters.add(fileSplitter);
                    fileSplitter.start();
                }
