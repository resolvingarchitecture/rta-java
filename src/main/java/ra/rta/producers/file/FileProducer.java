...
public class FileProducer {
	...
	public static void main(String... args) throws IOException {
        ...
        String partnerNamespace = args[0];
		int messageChunkSize = Integer.parseInt(args[1]);
        String kafkaURLList = args[2];
        String inboundFolder = args[3];
        String archiveFolder = args[4];

        LOG.info("Partner.name="+partnerNamespace);
		// Kafka Properties
		Properties transactionProducerProps = new Properties();
		Properties referenceProducerProps = new Properties();

		// Set up props
		Properties tProps = new Properties();
		tProps.put(ProducerConfig.CLIENT_ID_CONFIG, "TransactionProducer");
		tProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaURLList);
...
//        tProps.put(ProducerConfig.PARTITIONER_CLASS_CONFIG, "ra.rta.producers.utilities.partitioners.KafkaRoundRobinPartitioner");
		Producer<String,String> tProducer = new KafkaProducer<>(tProps);

		Properties dProps = new Properties();
		dProps.put(ProducerConfig.CLIENT_ID_CONFIG, "ReferenceProducer");
		dProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaURLList);
...
//        dProps.put(ProducerConfig.PARTITIONER_CLASS_CONFIG, "ra.rta.producers.utilities.partitioners.KafkaRoundRobinPartitioner");
		Producer<String,String> rProducer = new KafkaProducer<>(dProps);

        // Register Shutdown Hook with File Splitter list
        List<FileSplitter> fileSplitters = new ArrayList<>();
        Runtime.getRuntime().addShutdownHook(new FileProducerShutdown(fileSplitters, tProducer, rProducer));
			...
			for (Path path : oldFiles) {
				...
				if (path.toString().contains(".mkr")) {
                    ...
                    if(dataFile==null) {
                        ...
                        FileSplitter fileSplitter = new FileSplitter(partnerNamespace, dataFile.getFileName(), markerFile.getFileName(), startingLine, sourceDir, archiveDir, tProducer, rProducer);
...
//			WatchKey key = sourceDir.register(watcher, StandardWatchEventKinds.ENTRY_CREATE);
            ...
            for (;;) {
				...
				for (WatchEvent<?> event: key.pollEvents()) {
                    ...
                    FileSplitter fileSplitter = new FileSplitter(partnerNamespace, dateFileName, markerFileName, 0, sourceDir, archiveDir, tProducer, rProducer);
                    fileSplitters.add(fileSplitter);
                    fileSplitter.start();
				}
			...
			LOG.error("IOException in " + FileProducer.class.getSimpleName() + ".main()", e);
		}
